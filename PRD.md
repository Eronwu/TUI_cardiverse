...
> v0.4 修订声明：本文后续若仍出现 Node.js、npm、Ink、blessed 作为 MVP 主路线的表述，均视为历史原型描述。当前可执行路线以 Rust + Ratatui + Cargo 分发为准；TypeScript/npm 版本仅作为规则原型和迁移参考。

《Terminal Cardiverse》是一款以命令行为第一入口的开源策略游戏。体验爆点版切换为 Rust + Ratatui 原生终端驾驶舱，玩家通过 `cargo install` 安装，在真实终端中启动游戏，以自然语言 Prompt 即时编译战斗指令，在有限 RAM、有限内存插槽和严格规则校验下，与 AI Boss 的数字防御矩阵进行回合制攻防。
...
### 2.1 第一形态：Rust CLI / Ratatui Terminal TUI
...
```bash
cargo install terminal-cardiverse
cardiverse
```
...
历史 TypeScript/npm 版本保留为原型参考，后续如需要扩大分发面，可再增加 npm wrapper 包装 Rust 二进制：
...
### 7.4.1 Draft Card Flow

玩家输入 Prompt 后，系统先生成草稿卡，而不是立刻塞入 Cache。

```txt
> 用火焰刺穿它的外壳

DRAFT CARD
Thermal Spike / attack / cost 4 RAM
damage hp 16
tags: thermal, direct

[p] 立即使用  [c] 放入 Cache  [r <prompt>] 重写  [x] 丢弃
```
...
| OpenAI-compatible Env Key | 从 `LLM_API_BASE_URL` / `LLM_API_KEY` / `LLM_MODEL_NAME` 读取第三方或 OpenAI-compatible Provider | 支持 `/responses`，也可通过 `LLM_API_STYLE=chat_completions` 使用 `/chat/completions` |
| OpenAI Env Key | 从 `OPENAI_API_KEY` / `OPENAI_MODEL` 读取原生 OpenAI 配置 | 用户需要理解环境变量配置 |
...
- 输入 `help` 显示命令。
- 输入自然语言 Prompt 生成 Draft 卡。
- 输入 `p` 可立即使用 Draft 卡造成伤害。
- 输入 `end` 触发 Boss 回合。
- 玩家可击败 Boss。
- Boss 可击败玩家。
